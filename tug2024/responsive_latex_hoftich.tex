...
The \program{Rmodepdf} also detects and displays \LaTeX\ mathematical commands
embedded in web pages that use \program{MathJax} or \program{KaTeX} for
rendering. This default behavior can be disabled using the \texttt{-N} option.
Additionally, the removal of page elements using \program{Rdrview} can be
...
\program{Rmodepdf} provides the option to use a Lua configuration script. This
...
The font size can be set using the \verb|\setsizes{<number of characters
per|\allowbreak\verb|line>}| command. \tbcode{Responsive} tries to set the font
...
\end{subfigure}
  \caption{Example of typographic scales (default font size is highlighted with
  lines)}\label{fig:stupnice}
\end{figure*}
